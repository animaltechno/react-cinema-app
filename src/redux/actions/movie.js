...
export const getMovies = (type, pageNumber) => async dispatch => {
  try {
    ...
    // dispatch({ type: MOVIE_LIST, payload: result})
...
export const loadMoreMovies = (type, pageNumber) => async dispatch => {
  try {
    ...
    dispatchMethod(LOAD_MORE_RESULTS, {
      list: results, 
      page: payload.page, 
...
export const setResponseNumber = (page, totalPages ) => async (dispatch) => {
...
export const movieDetails = (id) => async (dispatch) => {
  try {
    ...
    // 上の5つのプロミスが終了したら次のプロミスが実行され、結果もarrayに収納される。(複数の非同期処理を並列に実行、その全てが成功した場合に処理を実行する)
